...
- [ ] 2.5 Keep `GET /health` unauthenticated.
...
- [ ] 3.7 Add regression coverage that `GET /health` remains accessible without an admin API key.

## 4. Documentation and API Contract

- [ ] 4.1 Update `docs/api/openapi.yaml` with the `X-Admin-API-Key` security scheme.
- [ ] 4.2 Mark protected operations as requiring admin API key authentication in OpenAPI.
- [ ] 4.3 Document HTTP `401 Unauthorized` responses for protected operations in OpenAPI.
