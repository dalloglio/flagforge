...
- Consolidate local non-secret environment defaults into `.env.example`, including test-only variables with the `TEST_` prefix, and remove the need for a committed `.env.test` file.
...
- Affected configuration: `.env.example`, removal of the committed `.env.test`, `docker-compose.yml`, and the PostgreSQL integration test environment loader.
