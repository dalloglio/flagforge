...
- Introduce a publish-capable GitHub Actions workflow for protected `main` branch pushes and manual `workflow_dispatch` runs.
...
- Define vulnerability scanning, reviewer expectations, rollback, retention, cleanup, cost, and sensitive-data guardrails for published images.
...
- ECR repository provisioning, account-backed plans, applies, destroys, imports, remote state, EKS, ALB, and production traffic routing remain outside this implementation scope.
