...
- Use the existing local container image, Helm chart, in-cluster PostgreSQL dependency, and operational endpoints as the basis for the cluster workflow.
- Add a basic smoke validation path that proves the API is reachable inside or through the local cluster.
- Document setup, reset, prerequisites, migration steps, and prominent local simulation limits.
...
- Affected systems: local platform documentation, kind configuration, thin Kubernetes workflow Makefile targets or documented commands, PostgreSQL setup/migration steps, and validation commands.
