...
Register the HTTP metrics middleware before request body parsing so it can observe responses generated by parser failures, including malformed JSON `400` responses. The middleware must attach its completion listener before later middleware can reject a request, while still deriving labels from stable Express route templates when available and stable fallback labels when parsing fails before route matching.
...
- Register metrics after body parsing. This misses malformed JSON client errors and weakens request/error-rate visibility, so it is not acceptable for the initial metrics behavior.
...
The documentation must also state that `/healthz` is a process liveness endpoint for a running API process, not a startup bypass. Runtime startup can still fail before the HTTP listener starts if required PostgreSQL configuration is missing, invalid, or unavailable.
...
- Metrics middleware can miss early middleware failures -> Register metrics before body parsing and test malformed JSON `400` observations explicitly.
- Liveness can be mistaken for startup independence -> Document that `/healthz` is only available after startup succeeds and the HTTP listener is serving.
...
4. Register metrics before body parsing so malformed JSON responses are observed.
5. Update OpenAPI for `GET /healthz` and `GET /readyz`.
6. Update README or local development runbook with verification commands and startup/liveness semantics.
7. Add focused tests for liveness, readiness success, readiness failure, metrics exposure, malformed JSON metrics coverage, unchanged `/health`, and OpenAPI validation.
8. Run `npm run verify` before marking implementation complete.
